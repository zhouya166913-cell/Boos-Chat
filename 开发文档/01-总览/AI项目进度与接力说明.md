...
3. 管理员在当前期数中创建分组，每组需要填写分组名称和组长，组长目前为手动填写。
4. 管理员在某个分组中录入学员，学员需要有座位号、姓名、新老学员类型和备注；手机号、身份证号可补充维护。
5. 学员通过二维码进入公开问卷时，需要填写姓名、手机号、身份证号，并在“我是新学员 / 我是老学员”中二选一。
6. 签到准入只校验姓名是否匹配当前期数已录入学员；手机号、身份证号和新老学员类型作为本次问卷资料保存，不参与准入拦截。
7. 姓名匹配成功后，立即视为签到成功；页面先展示“签到成功”、所属小组、组长和座位号。
8. 学员确认分组信息后，点击“进入 AI 诊断问卷”按钮再展开原调查问卷；姓名、手机号、身份证号和新老学员选择会自动带入，不再让学员重复填写基础身份信息。
9. 每次姓名匹配成功时，系统会在学员名单中累加进入次数，并据此显示“已签到/未签到”。
10. 学员提交问卷后，继续走原来的 AI 诊断分析流程，记录会归属到对应课程期数。
11. 课程管理页可以在同一页面查看当前期数的分组、学员名单、调查问卷记录、二维码和数据看板。
12. 数据看板新增“课程分析”按钮，会调用 `survey_solution_planner` 智能体汇总本期所有学员的痛点，生成给老师备课用的课程思路。
13. 同一学员可以重复填写问卷；每次提交都会保留历史记录，但课程管理、调查记录列表、数据看板和课程分析只展示或统计该学员最新一次提交。
...
GET  /api/admin/course-phases/{phaseId}/groups
POST /api/admin/course-phases/{phaseId}/groups
PUT  /api/admin/course-phases/{phaseId}/groups/{groupId}
DELETE /api/admin/course-phases/{phaseId}/groups/{groupId}
...
后续 `V35__course_student_checkin_stats.sql` 为 `course_student` 增加 `check_in_count` 和 `last_check_in_time`，用于记录学员成功签到的累计次数和最近签到时间。
后续 `V37__course_groups_and_student_number.sql` 新增 `course_group` 分组表，并为 `course_student` 增加 `group_id` 和 `student_no`，用于支持“期数 -> 分组 -> 组长 -> 学员”的课程组织结构。
后续 `V38__clear_survey_records_and_rename_seat_number.sql` 会先清空 `survey_record` 和 `course_analysis_history`，再把 `course_student.student_no` 改名为 `seat_no`，用于将“学号”正式调整为“座位号”。

发布后验证重点：

- 创建一期课程，确认二维码和问卷链接都带 `phase` 参数。
- 在当前期数下创建分组，确认每个分组都可以维护组长。
- 在分组中录入学员，确认座位号、姓名、备注、手机号、身份证号、新老学员类型都能保存。
- 用错误姓名访问问卷，必须被拦截；手机号、身份证号和新老学员类型虽必填，但不参与签到准入校验。
- 用正确学员姓名并完整填写手机号、身份证号、新老学员类型后，确认页面先显示签到成功和所属小组信息。
- 点击“进入 AI 诊断问卷”后，确认可以展开问卷，并且姓名、手机号、身份证号已自动带入。
- 签到成功后，课程管理的学员名单应显示“已签到”，进入次数应随每次成功签到累加。
...
- 2026-06-03 已手工清空本地 `boss_chat_dev` 的 `course_phase`、`course_group`、`course_student`、`survey_record`、`course_analysis_history`。
- 清理后上述 5 张表均为 0 条数据，用户、模型、智能体、图片存储等管理配置未清空。
- 下一步可以直接在本地从空白课程管理开始测试“创建期数 -> 创建分组 -> 录入学员 -> 姓名签到 -> 填写问卷 -> 数据看板课程分析”完整流程。
