...
7. 每次姓名匹配成功并进入问卷页时，系统会在学员名单中累加进入次数，并据此显示“已签到/未签到”。
8. 学员提交问卷后，继续走原来的 AI 诊断分析流程，记录会归属到对应课程期数。
9. 课程管理页可以在同一页面查看当前期数的学员名单、调查问卷记录、二维码和数据看板。
10. 数据看板新增“课程分析”按钮，会调用 `survey_solution_planner` 智能体汇总本期所有学员的痛点，生成给老师备课用的课程思路。
11. 同一学员可以重复填写问卷；每次提交都会保留历史记录，但课程管理、调查记录列表、数据看板和课程分析只展示或统计该学员最新一次提交。
