...
3. 管理员在当前期数中录入学员，字段包括姓名、手机号、身份证号、是否新学员、备注。
4. 学员通过二维码进入公开问卷时，只需要填写姓名完成签到校验。
5. 只有姓名匹配到当前期数已录入学员后，才会进入原调查问卷页面；姓名、手机号和身份证号会自动带入，不再让学员重复填写。
6. 学员提交问卷后，继续走原来的 AI 诊断分析流程，记录会归属到对应课程期数。
7. 课程管理页可以在同一页面查看当前期数的学员名单、调查问卷记录、二维码和数据看板。
8. 数据看板新增“课程分析”按钮，会调用 `survey_solution_planner` 智能体汇总本期所有学员的痛点，生成给老师备课用的课程思路。
...
- 录入学员后，用错误姓名访问问卷，必须被拦截；手机号和身份证号只作为学员资料与问卷记录字段，不再参与签到准入校验。
- 用正确学员姓名签到后，确认能进入问卷，且姓名、手机号和身份证号不再重复填写。
