...
4. 学员通过二维码进入公开问卷时，必须先填写姓名、手机号和身份证号完成签到校验。
5. 只有匹配到当前期数已录入学员后，才会进入原调查问卷页面；姓名、手机号和身份证号会自动带入，不再让学员重复填写。
...
- 录入学员后，用错误姓名、错误手机号、错误身份证号分别访问问卷，必须被拦截。
- 用正确学员信息签到后，确认能进入问卷，且姓名、手机号不再重复填写。
