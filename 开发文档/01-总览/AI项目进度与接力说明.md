...
7. 姓名匹配成功后，立即视为签到成功并进入原调查问卷页面；姓名、手机号、身份证号和新老学员选择会自动带入，不再让学员重复填写基础身份信息。
8. 每次姓名匹配成功并进入问卷页时，系统会在学员名单中累加进入次数，并据此显示“已签到/未签到”。
9. 学员提交问卷后，继续走原来的 AI 诊断分析流程，记录会归属到对应课程期数。
10. 课程管理页可以在同一页面查看当前期数的分组、学员名单、调查问卷记录、二维码和数据看板。
11. 数据看板新增“课程分析”按钮，会调用 `survey_solution_planner` 智能体汇总本期所有学员的痛点，生成给老师备课用的课程思路。
12. 同一学员可以重复填写问卷；每次提交都会保留历史记录，但课程管理、调查记录列表、数据看板和课程分析只展示或统计该学员最新一次提交。
...
后续 `V35__course_student_checkin_stats.sql` 为 `course_student` 增加 `check_in_count` 和 `last_check_in_time`，用于记录学员成功进入问卷页的累计次数和最近进入时间。
...
- 用正确学员姓名并完整填写手机号、身份证号、新老学员类型后，确认能进入问卷。
- 进入问卷成功后，课程管理的学员名单应显示“已签到”，进入次数应随每次成功进入问卷页累加。
