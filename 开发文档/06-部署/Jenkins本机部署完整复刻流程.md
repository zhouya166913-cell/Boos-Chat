...
当前阿里云 ECS 已经完成 Jenkins 本机部署闭环，Jenkins 实际端口已从 `8080` 改为 `9999`，部署任务为 `boss-chat-deploy`，后端服务由 `boss-chat.service` 托管，健康检查为 `http://127.0.0.1:9090/api/health`。

后续接手时优先阅读：[当前服务器部署 URL 与联调说明](./当前服务器部署URL与联调说明.md)。
